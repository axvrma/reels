	...
	template: `
		<div class="relative flex w-full items-center group-data-vertical:w-auto group-data-vertical:flex-col">
			...
			@for (i of _slider.thumbIndexes(); track i) {
				<span
					class="border-primary ring-ring/50 absolute block size-4 shrink-0 rounded-full border bg-white shadow-sm transition-[color,box-shadow] select-none after:absolute after:-inset-2 hover:ring-4 focus-visible:ring-4 focus-visible:outline-hidden"
					brnSliderThumb></span>
			}
		</div>
		...
		@if (_slider.showTicks()) {
			<div
				class="text-muted-foreground mt-3 flex w-full items-start justify-between gap-1 px-2 text-xs font-medium group-data-horizontal:group-data-inverted:flex-row-reverse group-data-vertical:ms-3 group-data-vertical:mt-0 group-data-vertical:w-auto group-data-vertical:flex-col-reverse group-data-vertical:px-0 group-data-vertical:py-2 group-data-vertical:group-data-inverted:flex-col">
				<div
